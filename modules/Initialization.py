...
def smart_random(S, r, R1, R2, R3):
    ...
    to S_trunc. This method draws r points in S_trunc and generates a tensor with rank <= r from them. The distribution
...
def smart_sample(S, r, R1, R2, R3):
    ...
    for l in range(r):
        X[:, l], Y[:, l], Z[:, l] = assign_values(S, X, Y, Z, R1, R2, R3, C1, C2, C3, arr1, arr2, arr3, l)

    return X, Y, Z


@jit(nogil=True)
def assign_values(S, X, Y, Z, R1, R2, R3, C1, C2, C3, arr1, arr2, arr3, l):
    """
